"""
Peter Lynch成长投资分析师代理 - 基于彼得·林奇的投资原则
Peter Lynch growth investing analyst agent - Based on Peter Lynch's investment principles
"""
...
class PeterLynchSignal(BaseModel):
    """
    Peter Lynch分析信号模型 - 包含投资信号、置信度和推理
    Peter Lynch analysis signal model - Contains investment signal, confidence and reasoning
    """
...
def peter_lynch_agent(state: AgentState):
    ...
    使用彼得·林奇的投资原则分析股票：
      - 投资你了解的企业（清晰、易懂的商业模式）
      - 合理价格的成长股（GARP），强调PEG比率
      - 寻找持续的收入和每股收益增长，以及可管理的债务
      - 警惕潜在的"十倍股"（高增长机会）
      - 避免过于复杂或高杠杆的企业
      - 使用新闻情绪和内部交易作为次要输入
      - 如果基本面强烈符合GARP，则更加积极
...
def analyze_lynch_fundamentals(financial_line_items: list) -> dict:
    ...
    # 2) Operating Margin
    om_values = [getattr(fi, 'operating_margin', None) for fi in financial_line_items if hasattr(fi, 'operating_margin') and getattr(fi, 'operating_margin', None) is not None]
    if om_values:
...
def generate_lynch_output(
    ...
    template = ChatPromptTemplate.from_messages(
        [
            (
                "system",
                """你是彼得·林奇的人工智能代理。你根据彼得·林奇的知名原则做出投资决策：
                
                1. 投资你了解的企业：强调易懂的商业模式，可能在日常生活中发现
                2. 合理价格的成长股（GARP）：依靠PEG比率作为主要指标
                3. 寻找"十倍股"：能够大幅增长收益和股价的公司
                4. 稳定增长：偏好持续的收入/收益扩张，较少关注短期噪音
                5. 避免高债务：注意危险的杠杆
                6. 管理层和故事：股票背后有好的"故事"，但不过度炒作或过于复杂
                
                当你提供推理时，请用彼得·林奇的语调：
                - 引用PEG比率
                - 如果适用，提及"十倍股"潜力
                - 参考个人或轶事观察（例如，"如果我的孩子们喜欢这个产品..."）
                - 使用实用、通俗的语言
                - 提供关键的正面和负面因素
                - 以明确的立场结束（买入、卖出或中性）
            ...
            (
                "human",
                """根据以下{ticker}的分析数据，产生你的彼得·林奇风格投资信号。
